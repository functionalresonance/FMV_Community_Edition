...
This is the basic version of the FRAM Model Visualiser.  
Branches from this master base will add other functionality, such as the FMI - FRAM Model Interpreter.  
The latest approved branch will be considered the standard version that is freely available as a Progressive Web App at https://fmv.zerprize.co.nz.
...
The application was developed as a Blazor WebAssembly App using Microsoft Visual Studio Community 2022, and published as a .NET 6.0 self-contained browser-wasm. If the repository was cloned to Visual Studio and published to a folder structure, the resulting web.config and wwwroot folder could be placed on a web server to run the application. 
